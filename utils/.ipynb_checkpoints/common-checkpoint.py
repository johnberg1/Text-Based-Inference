...
def vis_faces_difference(log_hooks, txt, mismatch_text):
	...
	for i in range(display_count):
		...
		vis_faces_with_difference(hooks_dict, fig, gs, i)
	plt.tight_layout()
	return fig

def vis_faces2(log_hooks, txt, mismatch_text):
	display_count = len(log_hooks)
	fig = plt.figure(figsize=(12, 4 * display_count))
	gs = fig.add_gridspec(display_count, 4)
	plt.gcf().text(0.02, 0.52, 'Original: {}'.format(txt[0]), fontsize=8)
	...
	for i in range(display_count):
		hooks_dict = log_hooks[i]
		vis_faces_with_age2(hooks_dict, fig, gs, i)
	plt.tight_layout()
	return fig
...
def vis_faces_two(log_hooks, txt, mismatch_text):
	display_count = len(log_hooks)
	fig = plt.figure(figsize=(12, 5 * display_count))
...
def vis_faces_with_age2(hooks_dict, fig, gs, i):
	...
	# plt.imshow(hooks_dict['trained-fixed'])
	# plt.title('Trained - Frozen')
    
def vis_faces_with_difference(hooks_dict, fig, gs, i):
	fig.add_subplot(gs[i, 0])
	plt.imshow(hooks_dict['input_face'])
	plt.title('Input')
	fig.add_subplot(gs[i, 1])
...
def vis_faces_sspace(log_hooks, txt, mismatch_text):
	display_count = len(log_hooks)
	fig = plt.figure(figsize=(12, 4 * display_count))
	gs = fig.add_gridspec(display_count, 4)
	plt.gcf().text(0.02, 0.52, 'Original: {}'.format(txt[0]), fontsize=8)
	...
	for i in range(display_count):
		hooks_dict = log_hooks[i]
		vis_faces_with_sspace(hooks_dict, fig, gs, i)
	plt.tight_layout()
	return fig
    
def vis_faces_with_sspace(hooks_dict, fig, gs, i):
	fig.add_subplot(gs[i, 0])
	plt.imshow(hooks_dict['input_face'])
	plt.title('Input\nOut Sim={:.2f}\n'.format(float(hooks_dict['diff_input_real'])))
	fig.add_subplot(gs[i, 1])
