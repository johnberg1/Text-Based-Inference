...
class Coach:
	...
	def perform_forward_pass(self, x, txt_embed):
		y_hat, latent, inversions, residuals = self.net.forward(x, txt_embed, return_latents=True)
		return y_hat, latent, inversions, residuals
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				y_hat, latent, inversions, residuals = self.perform_forward_pass(x, txt_embed_mismatch)
				loss, loss_dict, id_logs = self.calc_loss(x, y, y_hat, text_original, text_mismatch, latent, residuals, y, mismatch_text, data_type="real")
				loss.backward()
				...
				y_recovered, latent_cycle, inversions_cycle, residuals_cycle = self.perform_forward_pass(y_hat_clone, txt_embed_clone)
				loss, cycle_loss_dict, cycle_id_logs = self.calc_loss(x, y, y_recovered, text_mismatch, text_clone, latent_cycle, residuals_cycle, y_hat_clone, mismatch_text, data_type="cycle")
				...
				if self.global_step % self.opts.image_interval == 0 or \
					...
					self.parse_and_log_images(id_logs, x, y, y_hat, y_recovered, inversions, txt, mismatch_text,
	...
	def validate(self):
		...
		for batch_idx, batch in enumerate(self.test_dataloader):
			...
			with torch.no_grad():
				...
				# perform forward/backward pass on real images
				y_hat, latent, inversions, residuals = self.perform_forward_pass(x, txt_embed_mismatch)
				_, cur_loss_dict, id_logs = self.calc_loss(x, y, y_hat, text_original, text_mismatch, latent, residuals, y, mismatch_text, data_type="real")
			...
			# Logging related
			self.parse_and_log_images(id_logs, x, y, y_hat, y_recovered, inversions, txt, mismatch_text, title='images/test/faces',
	...
	def calc_loss(self, x, y, y_hat, source_text, target_text, latent, residuals, directional_source, mismatch_text, data_type="real"):
		loss_dict = {}
		id_logs = []
		loss = 0.0
		if self.opts.id_lambda > 0:
		...
		# loss_s_norm = torch.sum(latent.norm(2, dim=(1, 2))) / latent.shape[0]
		# print(loss_s_norm)
		# sys.exit("here")
		...
		for code in residuals: 
			code_norm = code.norm(2, dim=1)
			norms.append(code_norm)
		code_norms = torch.stack(norms, dim=0)
		loss_code_norms = torch.mean(code_norms.norm(2, dim=0))
		loss_dict['loss_residual_norm'] = float(loss_code_norms)
		loss += loss_code_norms * 0.005
	...
	def parse_and_log_images(self, id_logs, x, y, y_hat, y_recovered, inversions, txt, mismatch_text, title, subscript=None, display_count=2):
		im_data = []
		for i in range(display_count):
			cur_im_data = {
				'input_face': common.tensor2im(x[i]),
				'target_face': common.tensor2im(inversions[i]),
	...
	def log_images(self, name, txt, mismatch_text, im_data, subscript=None, log_latest=False):
		fig = common.vis_faces_sspace(im_data, txt, mismatch_text)
