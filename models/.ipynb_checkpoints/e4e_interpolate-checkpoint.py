...
from models.encoders import e4e_encoders_adain, e4e_encoders
...
class pSp(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.decoder = Generator(opts.output_size, 512, 8)
        self.face_pool = torch.nn.AdaptiveAvgPool2d((256, 256))
        # Load weights if needed
        self.load_weights()

    def set_encoder(self):
        encoder = e4e_encoders_adain.Encoder4Editing(50, 'ir_se', self.opts)
        return encoder

    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            print('Loading SAM over the pSp framework from checkpoint: {}'.format(self.opts.checkpoint_path))
    ...
    def forward(self, x, txt_embed, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
        ...
        if input_code: 
            ...
            # normalize with respect to the latent of the encoded image of pretrained pSp encoder
                ...
                with torch.no_grad():
                    ...
                    encoded_latents = encoded_latents + self.latent_avg.repeat(codes.shape[0], 1, 1)
        ...
        if return_latents:
            return images, result_latent, codes, psp_out
        else:
            return images, codes, psp_out
    ...
    def __get_pretrained_psp_encoder(self):
        ...
        encoder = e4e_encoders.Encoder4Editing(50, 'ir_se', opts_encoder)
        return encoder
    
    def __load_pretrained_psp_encoder(self):
        print(f'Loading e4e encoder from checkpoint: {self.opts.pretrained_psp_path}')
