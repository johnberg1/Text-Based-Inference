...
#         #self.channelwise_alpha = nn.Parameter(torch.full((in_channel,spatial,spatial),-2.0))
#         #self.act = nn.Sigmoid()
...
#         out = gamma * out + beta
#         out += skip
#         #alpha = self.act(self.channelwise_alpha)
#         #out = out * alpha + input * (1 - alpha)
#         return out

# RESIDUAL ADAPTER ADAIN (https://arxiv.org/pdf/1705.08045.pdf)

# def conv1x1_fonc(in_planes, stride=1, bias=False):
...
#         self.conv = nn.Conv2d(planes, planes, kernel_size=1, stride=stride, padding=0, bias=False)
...
#         self.style_mapper = nn.Sequential(nn.Linear(style_dim, style_dim), nn.ReLU(), nn.Linear(style_dim, style_dim), nn.ReLU(), nn.Linear(style_dim, style_dim), nn.ReLU())
#         self.style_out = nn.Linear(style_dim, in_channel * 2)
#         self.conv = conv1x1(in_channel)
#         self.conv2 = conv1x1(in_channel)
#         self.act = nn.ReLU()
...
#         out = self.act(out)
#         out = self.bn(out)
#         # out = gamma * self.norm(out) + beta
#         out = self.conv2(out)
        
#         out += skip
#         return out

#  EXAGGERATION (https://arxiv.org/pdf/2107.04331.pdf)
